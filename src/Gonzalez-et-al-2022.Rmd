...
In this analysis, we will conduct an ordinal multiple linear regression on **s19 (self-reported substance use)** to see how it changes with respect to three predictor variables: **social support from an adult at home**, **social support from at least one peer**, **social support from a teacher or counselor at school**, **ptsd_symptom_total**, **grade**, and **gender**. 

Here's our model: 

```{r ordinal_regression}
clm_with_interaction <- clm(s19 ~  q20 + q21 + q22 +
...
polr_lm_with_interaction <- polr(s19 ~  q20 + q21 + q22 +
                              ptsd_symptom_total + grado + genero + 
                              genero*ptsd_symptom_total + genero*q20 + 
                              genero*q21 + genero*q22, 
                            data = ordinalRegres_substanceUse, 
...
For more information on interpreting odds ratios see: [How do I interpret odds ratios in logistic regression?](https://rpubs.com/raoulbia/interpreting_glm_logistic_regression_output). The odds ratios for the intercepts are not generally interpreted. 
